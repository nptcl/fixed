...
(defconstant +elliptic-secp384r1+
  '(384
...
(defconstant +elliptic-secp521r1+
  '(521
...
(defconstant +elliptic-curve25519+
  '(256
    #x7FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFED
    #x76D06
    #x01
...
(defvar *elliptic-encode3*)
(defvar *elliptic-decode1*)
(defvar *elliptic-decode2*)
(defvar *elliptic-decode3*)
...
(defun make-private-512bit (n)
  (let ((sha (make-sha512encode)))
    (dotimes (i n)
      (little-endian-sha512encode sha (random (ash 1 512)) 48))
    (vector-little-integer
      (calc-sha512encode sha))))

(defun make-private-384bit (&optional (n 4))
  (ldb (byte 384 0) (make-private-512bit n)))

(defun make-private-521bit (&optional (n 4))
  (let ((sha (make-shake-256-encode)))
    (dotimes (i n)
      (little-endian-sha3encode sha (random (ash 1 512)) 64))
    (ldb (byte 521 0)
         (vector-little-integer
           (result-sha3encode sha 66)))))

(defun make-private-secp256k1 ()
  (let ((x (make-private-256bit)))
    (if (< 0 x *elliptic-n*)
      x
      (make-private-secp256k1))))

(defun make-private-secp384r1 ()
  (let ((x (make-private-384bit)))
    (if (< 0 x *elliptic-n*)
      x
      (make-private-secp384r1))))

(defun make-private-secp521r1 ()
  (let ((x (make-private-521bit)))
    (if (< 0 x *elliptic-n*)
      x
      (make-private-secp521r1))))
...
(defun encode1-secp384r1 (v)
  (integer-big-vector v 48))

(defun encode1-secp521r1 (v)
  (integer-big-vector v 66))
...
(defun encode3-secp256r1 (x)
  (integer-big-vector x 32))

(defun encode3-secp384r1 (x)
  (integer-big-vector x 48))

(defun encode3-secp521r1 (x)
  (integer-big-vector x 66))
...
(defun decode3-secp256r1 (x)
  (vector-big-integer x))

(defun decode3-secp384r1 (x)
  (vector-big-integer x))

(defun decode3-secp521r1 (x)
  (vector-big-integer x))
  ...
  (destructuring-bind (bit p a b (gx gy) n h) +elliptic-secp256r1+
    `(with-elliptic-weierstrass
       ...
       (let* ((*elliptic-curve* :secp256r1)
              ...
              (*elliptic-encode3* #'encode3-secp256r1)
              (*elliptic-decode1* #'decode1-secp256r1)
              (*elliptic-decode2* #'decode2-secp256r1)
              (*elliptic-decode3* #'decode3-secp256r1))
         ,@body))))

(defmacro with-elliptic-secp384r1 (&body body)
  (destructuring-bind (bit p a b (gx gy) n h) +elliptic-secp384r1+
    `(with-elliptic-weierstrass
       (,bit ,p ,gx ,gy ,n ,h)
       (let* ((*elliptic-curve* :secp384r1)
              (*elliptic-a* ,a)
              (*elliptic-b* ,b)
              (*elliptic-addition* #'addition-secp256k1)
              (*elliptic-doubling* #'doubling-secp256k1)
              (*elliptic-make-private* #'make-private-secp384r1)
              (*elliptic-make-public* #'make-public-secp256k1)
              (*elliptic-encode3* #'encode3-secp384r1)
              (*elliptic-decode3* #'decode3-secp384r1))
         ,@body))))

(defmacro with-elliptic-secp521r1 (&body body)
  (destructuring-bind (bit p a b (gx gy) n h) +elliptic-secp521r1+
    `(with-elliptic-weierstrass
       (,bit ,p ,gx ,gy ,n ,h)
       (let* ((*elliptic-curve* :secp521r1)
              (*elliptic-a* ,a)
              (*elliptic-b* ,b)
              (*elliptic-addition* #'addition-secp256k1)
              (*elliptic-doubling* #'doubling-secp256k1)
              (*elliptic-make-private* #'make-private-secp521r1)
              (*elliptic-make-public* #'make-public-secp256k1)
              (*elliptic-encode3* #'encode3-secp521r1)
              (*elliptic-decode3* #'decode3-secp521r1))
         ,@body))))
...
(defvar *elliptic-x1*)
(defvar *elliptic-a24*)
(defvar *elliptic-square-root*)
(defvar *elliptic-scalar1*)
(defvar *elliptic-scalar2*)

(defstruct (pointz (:constructor empty-pointz)) x z)

(defun make-pointz (x z)
  (empty-pointz :x x :z z))

(defun scalar1-curve25519 (k)
  (logior (ash 1 254)
          (mask-field (byte 251 3) k)))

(defun scalar1-curve448 (k)
  (logior (ash 1 447)
          (mask-field (byte 445 2) k)))

(defun scalar1 (k)
  (funcall *elliptic-scalar1* k))

(defun scalar2-curve25519 (k)
   (mask-field (byte 255 0) k))

(defun scalar2-curve448 (k)
   (mask-field (byte 448 0) k))

(defun scalar2 (k)
   (funcall *elliptic-scalar2* k))

(defun encode3-curve25519 (x)
  (integer-little-vector x 32))

(defun encode3-curve448 (x)
  (integer-little-vector x 56))

(defun decode3-curve25519 (x)
  (vector-little-integer x))

(defun decode3-curve448 (x)
  (vector-little-integer x))

(defun encode3-vector (x)
  (funcall *elliptic-encode3* x))

(defun decode3-vector (x)
  (when x
    (funcall *elliptic-decode3* x)))

(defun encode3-string (x)
  (encode-string
    (encode3-vector x)))

(defun decode3-string (x)
  (decode3-vector
    (decode-string x)))

(defun square-root (x)
  (funcall *elliptic-square-root* x))

(defun valid-montgomery (a)
  (let* ((x (point2-x a))
         (y (point2-y a)))
    (zerop
      (modp (- (* y y)
               (+ (* x x x) (* *elliptic-a* x x) x))))))

;;  addition
(defun addition-doubling-montgomery (v2 v3)
  (let* ((x2 (pointz-x v2))
         (z2 (pointz-z v2))
         (x3 (pointz-x v3))
         (z3 (pointz-z v3))
         ...
         (z3 (- da cb))
         (add-x (* x3 x3))
         (add-z (* *elliptic-x1* z3 z3))
         (a2 (* a1 a1))
         (b2 (* b1 b1))
         (e1 (- a2 b2))
         (dbl-x (* a2 b2))
         (dbl-z (* e1 (+ a2 (* *elliptic-a24* e1)))))
    (values (make-pointz (modp dbl-x) (modp dbl-z))
            (make-pointz (modp add-x) (modp add-z)))))

(defun multiple-montgomery-x (k u)
  (let ((*elliptic-x1* u)
        (x (make-pointz 1 0))
        (y (make-pointz u 1))
        (swap 0))
    (loop for i from (1- *elliptic-bit*) downto 0 do
          ...
          (multiple-value-setq (x y)
            (addition-doubling-montgomery x y)))
    (unless (zerop swap)
      (rotatef x y))
    (mulp (pointz-x x)
          (inverse (pointz-z x)))))

(defun multiple-montgomery-y (x)
  ;;  y^2 = x^3 + A*x^2 + x
  (square-root
    (modp (+ (* x x x) (* *elliptic-a* x x) x))))

(defun multiple-montgomery (k u)
  (let* ((x (multiple-montgomery-x k u))
         (y (multiple-montgomery-y x)))
    (make-point2 x y)))

(defun make-public-montgomery (k)
  (multiple-montgomery k (point2-x *elliptic-g*)))

;;  macro
(defmacro with-elliptic-montgomery ((bit p gx gy n h) &body body)
  `(let* ((*elliptic-bit* ,bit)
          (*elliptic-p* ,p)
          (*elliptic-g* (make-point2 ,gx ,gy))
          (*elliptic-n* ,n)
          (*elliptic-h* ,h)
          (*elliptic-o* (make-pointz 1 0))
          (*elliptic-valid* #'valid-montgomery)
          (*elliptic-neutral* nil))
     ,@body))

(defmacro with-elliptic-curve25519 (&body body)
  (destructuring-bind (bit p a b (gx gy) n h) +elliptic-curve25519+
    `(with-elliptic-montgomery
       (,bit ,p ,gx ,gy ,n ,h)
       (let* ((*elliptic-curve* :curve25519)
              (*elliptic-a* ,a)
              (*elliptic-b* ,b)
              (*elliptic-a24* 121665)
              (*elliptic-square-root* #'square-root-mod-8)
              (*elliptic-scalar1* #'scalar1-curve25519)
              (*elliptic-scalar2* #'scalar2-curve25519)
              (*elliptic-make-private* #'make-private-ed25519)
              (*elliptic-make-public* #'make-public-montgomery)
              (*elliptic-encode3* #'encode3-curve25519)
              (*elliptic-decode3* #'decode3-curve25519))
         ,@body))))

(defmacro with-elliptic-curve448 (&body body)
  (destructuring-bind (bit p a b (gx gy) n h) +elliptic-curve448+
    `(with-elliptic-montgomery
       (,bit ,p ,gx ,gy ,n ,h)
       (let* ((*elliptic-curve* :curve448)
              (*elliptic-a* ,a)
              (*elliptic-b* ,b)
              (*elliptic-a24* 39081)
              (*elliptic-square-root* #'square-root-mod-4)
              (*elliptic-scalar1* #'scalar1-curve448)
              (*elliptic-scalar2* #'scalar2-curve448)
              (*elliptic-make-private* #'make-private-ed448)
              (*elliptic-make-public* #'make-public-montgomery)
              (*elliptic-encode3* #'encode3-curve448)
              (*elliptic-decode3* #'decode3-curve448))
         ,@body))))
...
(defun dhweierstrass-init ()
  (let* ((k (make-private))
         (a (make-public k)))
    (make-diffie-hellman :k k :a a)))

(defun dhmontgomery-init ()
  (dhweierstrass-init))

(defun dhweierstrass-send (s)
  (let* ((p (diffie-hellman-a s))
         (a (affine p)))
    (values
      (encode3-vector (point2-x a))
      (encode3-vector (point2-y a)))))

(defun dhmontgomery-send (s)
  (let ((a (diffie-hellman-a s)))
    (values
      (encode3-vector (point2-x a))
      (encode3-vector (point2-y a)))))

(defun dhweierstrass-reveice (s x y)
  (let ((x (decode3-vector x))
        (y (decode3-vector y)))
    (and (< x *elliptic-p*)
         (< y *elliptic-p*)
         (let ((b (make-point3 x y)))
           (when (valid b)
             (setf (diffie-hellman-b s) b)
             b)))))

(defun dhmontgomery-reveice (s x y)
  (let ((x (decode3-vector x))
        (y (decode3-vector y)))
    (and (< x *elliptic-p*)
         (< y *elliptic-p*)
         (let ((b (make-point2 x y)))
           (when (valid b)
             (setf (diffie-hellman-b s) b)
             b)))))

(defun dhweierstrass-shared (s)
  (let ((b (diffie-hellman-b s)))
    (when b
      (let* ((k (diffie-hellman-k s))
             (c (multiple k b))
             (x (point3-x c)))
        (unless (zerop x)
          (setf (diffie-hellman-c s) c)
          c)))))

(defun dhmontgomery-shared (s)
  (let ((b (diffie-hellman-b s)))
    (when b
      (let* ((k (diffie-hellman-k s))
             (u (point2-x b))
             (c (multiple-montgomery k u))
             (x (point2-x c)))
        (unless (zerop x)
          (setf (diffie-hellman-c s) c)
          c)))))

(defun dhweierstrass-output (s)
  (let ((c (diffie-hellman-c s)))
    (when c
      (let ((a (affine c)))
        (values
          (encode3-vector (point2-x a))
...
(defun dhmontgomery-output (s)
  (let ((c (diffie-hellman-c s)))
    (when c
      (values
        (encode3-vector (point2-x c))
        (encode3-vector (point2-y c))))))
