    ...
    (multiple-value-bind (size tail) (truncate (length msg) 16)
      (dotimes (i size)
        (let* ((x (* i 16))
          ...
          (setq n (logior n (ash 1 128)))
          (setq a (mod (* r (+ a n)) +poly1305-prime+))))
      (unless (zerop tail)
        (let* ((x (* size 16))
               (y (+ x tail))
               (n (vector-little-integer msg :start x :end y)))
          (setq n (logior n (ash 1 (* 8 tail))))
          (setq a (mod (* r (+ a n)) +poly1305-prime+)))))
    (incf a s)
...
(defun poly1305-ahead-encrypt-body (aad cipher)
  (concatenate
    'vector
    aad (pad16 aad)
    cipher (pad16 cipher)
...
(defun poly1305-ahead-encrypt (aad key nonce input)
  (let ((c (make-chacha20)))
    (let* ((otk (poly1305-key c key nonce))
           ...
           (body (poly1305-ahead-encrypt-body aad cipher))
